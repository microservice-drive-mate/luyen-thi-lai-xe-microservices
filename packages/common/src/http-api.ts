...
import type { ArgumentsHost, CallHandler, ExecutionContext, ExceptionFilter } from '@nestjs/common';
...
export class ApiResponseInterceptor<T>
  implements NestInterceptor<T, { success: boolean; code: string; message: string; timestamp: string; path: string; data: T }>
{
  intercept(
    context: ExecutionContext,
    next: CallHandler<T>,
  ): Observable<{ success: boolean; code: string; message: string; timestamp: string; path: string; data: T }> {
    if (context.getType() !== 'http') {
      return next.handle() as Observable<{ success: boolean; code: string; message: string; timestamp: string; path: string; data: T }>;
    }
    ...
    return next.handle().pipe(
      map((data) => {
        if (
          ...
          return data as unknown as { success: boolean; code: string; message: string; timestamp: string; path: string; data: T };
        }

        return {
          success: true,
          code: 'SUCCESS',
          message: response.statusCode === HttpStatus.CREATED ? 'Created' : 'OK',
